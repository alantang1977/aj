...
用法示例：
    python clun_test.py ping                     # 站点连通性/健康检查
    python clun_test.py hot                      # 抓取首页热播榜（按分区）
    python clun_test.py type 2 -p 1 -n 5         # 抓取剧集分类前 5 页
    python clun_test.py search 死神              # 搜索关键词
    python clun_test.py detail 714893571         # 抓取详情页（含播放源/网盘链接）
    python clun_test.py detail 714893571 -json   # 详情输出 JSON
    python clun_test.py crawl 2 -p 1 -n 10 -o out.json   # 分类批量抓取并落盘
